  ...
  const fetchWorkCenters = async () => {
    try {
      ...
      if (workCenters && workCenters.length > 0) {
        ...
        console.warn('No se encontraron centros de trabajo para este supervisor');
        setWorkCenters([]);
      }
    } catch (error) {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (employeesData) {
        ...
        console.warn('No se encontraron empleados para los centros de trabajo del supervisor');
        setEmployees([]);
      }
    } catch (error) {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      switch (reportType) {
        ...
        case 'daily': {
          reportData = workCenterEmployees.map(employee => {
