  ...
  useEffect(() => {
    const fetchSupervisorDelegation = async () => {
      try {
        ...
        if (!supervisorEmail) throw new Error('No se encontró el email del supervisor');
        ...
        if (!data) throw new Error('No se encontró el perfil de supervisor');

        // Get the first valid delegation from the array
        const validDelegation = data.delegations && data.delegations.length > 0
          ? data.delegations.find((d: string) => delegationOptions.includes(d))
          : null;

        if (!validDelegation) {
          throw new Error('No se encontró una delegación válida para el supervisor');
        }

        setDelegation(validDelegation);
      } catch (err) {
        console.error('Error al obtener la delegación:', err);
        setError('Error al cargar la información de la delegación');
      }
    };

    fetchSupervisorDelegation();
  }, []);
